...
def initialize_parameters():
    ...
    W1 = tf.get_variable("W1", [60,19545], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    ...
    with tf.Session() as sess:
      ...
      for epoch in range(num_epochs):
        ...
        if print_cost and epoch % 10 == 0:
            print ("Cost after epoch %i: %f correlation coefficient:" %(epoch, np.sqrt(epoch_cost)))
        costs.append(np.sqrt(epoch_cost))
      val_cost=0
      ...
      print("Cost-validation data : %f correlation coefficient-validation data: %f" % (np.sqrt(val_cost),val_corr))
...
def train_model(train_file='\BigX1.mat',train_y='\Y_train.mat', job_dir='./tmp/crop-challenge', num_ep=100,**args):
    logs_path = job_dir + '/logs/' + datetime.now().isoformat()
    file_stream = file_io.FileIO(train_file, mode='r')
    f = h5py.File(file_stream)
    file_streamY = file_io.FileIO(train_y, mode='r')
    dataY= sio.loadmat (file_streamY)
    X_train= f['BigX1'][:]
    n_sample=500
    X_train=X_train[:,0:n_sample]
    n_fold=10
    cv=KFold(n_splits=n_fold, shuffle=True, random_state=None)
    Y_train = dataY['Y_train'][0:n_sample]
    ...
    for k,(train_index, val_index) in enumerate(cv.split(X_train)):
        ...
        parameters, val_cost, val_corr = nn_model(XX_train, YY_train,XX_val,YY_val, 1000, learning_rate=0.0015, minibatch_size=100, print_cost=True)
        record[k,1]=val_corr
