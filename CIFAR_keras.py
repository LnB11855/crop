...
def train_model(batch_size=1000,epochs=100,num_samples=10000):
    ...
    if K.image_data_format() == 'channels_first':
        x_train = x_train.reshape(x_train.shape[0], nchannel, img_rows, img_cols)
        x_test = x_test.reshape(x_test.shape[0], nchannel, img_rows, img_cols)
        input_shape = (nchannel, img_rows, img_cols)
    else:
        x_train = x_train.reshape(x_train.shape[0], img_rows, img_cols, nchannel)
        x_test = x_test.reshape(x_test.shape[0], img_rows, img_cols, nchannel)
        input_shape = (img_rows, img_cols, nchannel)
    ...
    df.to_csv ('CIFAR10_keras'+str(num_samples)+str(epochs)+'.csv', index = None, header=True)
